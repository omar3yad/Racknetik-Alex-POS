...
# ----------------------------------------------------
# Operator Direct Subscription Registration & Receipt
# ----------------------------------------------------
...
async def operator_subscription_create_submit(
    request: Request,
    full_name: str = Form(...),
    plate_number: str = Form(...),
    ...
    if not cleaned_card_code:
        return templates.TemplateResponse(
            "operator/subscription_new.html",
            {
                ...
                "shift": shift,
                "plans": plans,
                "error": "يرجى إدخال أو مسح كود الكارت",
                "form_data": form_data,
            },
            status_code=400,
        )
    ...
    try:
        ...
        if card.status != CardStatus.AVAILABLE:
            raise CardNotAvailableError(f"الكارت '{cleaned_card_code}' مستخدم حالياً أو غير متاح")

        # 2. Normalize Plate & Find or Create Subscriber
        norm_plate = plate_service.normalize(plate_number)
        subscriber = await subscriber_repo.get_by_plate(norm_plate)
        if not subscriber:
            subscriber = await subscriber_service.create_subscriber(
                SubscriberCreate(
        ...
        # 3. Fetch Plan & Calculate Price
        plan = await plan_repo.get_by_id(plan_id)
        if not plan:
            raise PlanNotFoundError("باقة الاشتراك المحددة غير موجودة")
        if not plan.is_active:
            raise PlanNotActiveError("باقة الاشتراك المحددة غير نشطة")
...
@router.get("/subscriptions/receipt/{subscription_id}")
async def operator_subscription_receipt_page(
    request: Request,
    subscription_id: int,
    current_user: User = Depends(require_operator),
    db: AsyncSession = Depends(get_db),
    ...
    result = await db.execute(select(ParkingCard).where(ParkingCard.id == subscription.card_id).limit(1))
    ...
    return templates.TemplateResponse(
        "operator/subscription_receipt.html",
        {
            "request": request,
            "subscription": subscription,
            "subscriber": subscriber,
